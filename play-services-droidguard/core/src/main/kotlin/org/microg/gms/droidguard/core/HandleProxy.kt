...
class HandleProxy(val handle: Any, val vmKey: String, val extra: ByteArray = ByteArray(0)) {
    constructor(clazz: Class<*>, context: Context, vmKey: String, data: Parcelable) : this(
        kotlin.runCatching {
            clazz.getDeclaredConstructor(Context::class.java, Parcelable::class.java)
    ...
    constructor(
        ...
        kotlin.runCatching {
            clazz.getDeclaredConstructor(
                ...
                Object::class.java,
                Bundle::class.java
            ).newInstance(context, flow, byteCode, callback, bundle)
        }.getOrElse {
    ...
    fun run(data: Map<Any, Any>): ByteArray {
        try {
            return handle.javaClass.getDeclaredMethod("run", Map::class.java)
                .invoke(handle, data) as ByteArray
        } catch (e: Exception) {
