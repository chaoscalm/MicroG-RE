...
class ExposureNotificationServiceImpl(
...
) : INearbyExposureNotificationService.Stub(), LifecycleOwner {
    ...
    private val backendPublicKey = backendPubKeyForPackage[packageName]?.let {
        try {
            KeyFactory.getInstance("EC")
                .generatePublic(X509EncodedKeySpec(Base64.decode(it, Base64.DEFAULT)))
        } catch (e: Exception) {
    ...
    private fun LifecycleCoroutineScope.launchSafely(block: suspend CoroutineScope.() -> Unit): Job =
        launchWhenStarted {
            try {
                block()
            } catch (e: Exception) {
                Log.w(TAG, "Error in coroutine", e)
            }
        }
        ...
        return ExposureDatabase.with(context) { database ->
            when {
                ...
                !hasConfirmActivity() -> {
                    ...
                    Status(
                        RESOLUTION_REQUIRED,
                        "Permission EN#$permission required.",
                        pendingConfirm(permission)
                    )
                }
            }
        }
        ...
        lifecycleScope.launchSafely {
            ...
            try {
                params.callback.onResult(
                    Status.SUCCESS,
                    isAuthorized && ExposurePreferences(context).enabled
                )
            } catch (e: Exception) {
                Log.w(TAG, "Callback failed", e)
            }
    ...
    private fun TemporaryExposureKeyProto.toKey(): TemporaryExposureKey =
        TemporaryExposureKey.TemporaryExposureKeyBuilder()
            .setKeyData(
                key_data?.toByteArray() ?: throw IllegalArgumentException("key data missing")
            )
            ...
            .setRollingPeriod(
                rolling_period ?: throw IllegalArgumentException("rolling period missing")
            )
    ...
    private suspend fun buildExposureSummary(token: String): ExposureSummary =
        ExposureDatabase.with(context) { database ->
            ...
            ExposureSummary.ExposureSummaryBuilder()
                .setDaysSinceLastExposure(
                    exposures.map { it.daysSinceExposure }.minOrNull()?.toInt() ?: 0
                )
                .setMatchedKeyCount(exposures.map { it.key }.distinct().size)
                .setMaximumRiskScore(
                    exposures.map { it.getRiskScore(configuration) }.maxOrNull()?.toInt() ?: 0
                )
            ...
            ExposureDatabase.with(context) { database ->
                ...
                for (file in params.keyFiles.orEmpty()) {
                    try {
                        val cacheFile = File(
                            context.cacheDir,
                            "en-keyfile-${System.currentTimeMillis()}-${Random.nextInt()}.zip"
                        )
                        ParcelFileDescriptor.AutoCloseInputStream(file)
                            .use { it.copyToFile(cacheFile) }
                        ...
                        while (keyFileSupplier.isAvailable && keyFileSupplier.hasNext()) {
                            withContext(Dispatchers.IO) {
                                try {
                                    val cacheFile = File(
                                        context.cacheDir,
                                        "en-keyfile-${System.currentTimeMillis()}-${Random.nextLong()}.zip"
                                    )
                                    ParcelFileDescriptor.AutoCloseInputStream(keyFileSupplier.next())
                                        .use { it.copyToFile(cacheFile) }
                                    val hash =
                                        MessageDigest.getInstance("SHA-256").digest(cacheFile)
                                    val storedKeys = database.storeDiagnosisFileUsed(tid, hash)
                                    if (storedKeys != null) {
                                        keys += storedKeys.toInt()
                                        cacheFile.delete()
                ...
                if (todoKeyFiles.size > 0) {
                    val time =
                        (System.currentTimeMillis() - start).coerceAtLeast(1).toDouble() / 1000.0
                    Log.d(
                        TAG,
                        "$packageName/$token processed $keys keys (${todoKeyFiles.size} files pending) in ${time}s -> ${
                            (keys.toDouble() / time * 1000).roundToInt().toDouble() / 1000.0
                        } keys/s"
                    )
                }
                                ...
                                for (entry in zip.entries()) {
                                    if (entry.name == "export.bin") {
                                        ...
                                        if (totalBytesRead == prefix.size && String(prefix).trim() == "EK Export v1") {
                                            val export =
                                                TemporaryExposureKeyExport.ADAPTER.decode(stream)
                                            database.finishFileMatching(
                                                tid,
                                                hash,
                                                export.end_timestamp?.let { it * 1000 }
                                                    ?: System.currentTimeMillis(),
                                                export.keys.map { it.toKey() },
                                                export.revised_keys.map { it.toKey() })
                ...
                Log.d(
                    TAG,
                    "$packageName/$token processed $keys keys ($newKeys new) in ${time}s -> ${
                        (keys.toDouble() / time * 1000).roundToInt().toDouble() / 1000.0
                    } keys/s"
                )
        ...
        try {
            ZipFile(file).use { zip ->
                ...
                for (sig in sigList.signatures) {
                    ...
                    val sigInfo =
                        sig.signature_info ?: throw Exception("Signature information is missing")
                    Log.d(
                        TAG,
                        "Signature info: algo=${sigInfo.signature_algorithm} key={id=${sigInfo.verification_key_id}, version=${sigInfo.verification_key_version}}"
                    )

                    val signature = sig.signature?.toByteArray()
                        ?: throw Exception("Signature contents is missing")
                    val sigVerifier = (sigAlgoForOid.get(sigInfo.signature_algorithm)
                        ?: throw Exception("Signature algorithm not supported: ${sigInfo.signature_algorithm}"))()
    ...
    private fun List<MergedSubExposure>.toScanInstances(): List<ScanInstance> {
        ...
        for (subExposure in this) {
            ...
            if (subExposure.duration > 5 * 60 * 1000L) {
                res.add(
                    ScanInstance.Builder().apply(subExposure).setSecondsSinceLastScan(
                        (subExposure.duration - 5 * 60).coerceAtMost(5 * 60).toInt()
                    ).build()
                )
            }
        }
            ...
            val response = getExposureWindowsInternal().groupBy { it.dateMillisSinceEpoch }.map {
                ...
                for (entry in it.value.groupBy { it.reportType }) {
                    for (window in entry.value) {
                        val weightedDuration = window.scanInstances.map {
                            params.config.weightedDurationFor(
                                it.typicalAttenuationDb,
                                it.secondsSinceLastScan
                            )
                        }.sum()
                        val score = (params.config.reportTypeWeights[window.reportType] ?: 1.0) *
                                (params.config.infectiousnessWeights[window.infectiousness]
                                    ?: 1.0) *
                                weightedDuration
                        if (score >= params.config.minimumWindowScore) {
                            map[entry.key] = DailySummary.ExposureSummaryData(
                                max(
                                    map[entry.key].maximumScore,
                                    score
                                ),
                                map[entry.key].scoreSum + score,
                                map[entry.key].weightedDurationSum + weightedDuration
                            )
                        }
                    }
                }
    ...
    override fun getPackageConfiguration(params: GetPackageConfigurationParams) {
        ...
        lifecycleScope.launchSafely {
            ...
            try {
                params.callback.onResult(
                    Status.SUCCESS,
                    PackageConfiguration.PackageConfigurationBuilder().setValues(Bundle.EMPTY)
                        .build()
                )
            } catch (e: Exception) {
                Log.w(TAG, "Callback failed", e)
    ...
    override fun getStatus(params: GetStatusParams) {
        lifecycleScope.launchSafely {
            ...
            try {
                params.callback.onResult(
                    Status.SUCCESS,
                    ExposureNotificationStatus.setToFlags(flags)
                )
            } catch (e: Exception) {
                Log.w(TAG, "Callback failed", e)
            }
