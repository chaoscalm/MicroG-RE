...
data class DeviceInfo(
    ...
    val device: String,
    val model: String,
    val rssiCorrection: Byte,
    val txPowerCorrection: Byte,
    @CalibrationConfidence val confidence: Int = CalibrationConfidence.MEDIUM
)

private var knownDeviceInfo: DeviceInfo? = null

fun averageCurrentDeviceInfo(
    oem: String,
    device: String,
    model: String,
    deviceInfos: List<DeviceInfo>,
    @CalibrationConfidence confidence: Int = CalibrationConfidence.LOW
): DeviceInfo =
    DeviceInfo(
        oem,
        device,
        model,
        deviceInfos.map { it.rssiCorrection }.average().roundToInt().toByte(),
        deviceInfos.map { it.txPowerCorrection }.average().roundToInt().toByte(),
        confidence
    )
...
val currentDeviceInfo: DeviceInfo
    get() {
        ...
        if (deviceInfo == null) {
            ...
            val exactMatch = byOem.find {
                it.device.equalsIgnoreCase(Build.DEVICE) && it.model.equalsIgnoreCase(Build.MODEL)
            }
            deviceInfo = when {
                exactMatch != null -> {
                ...
                byModel.isNotEmpty() || byDevice.isNotEmpty() -> {
                    ...
                    averageCurrentDeviceInfo(
                        Build.MANUFACTURER,
                        Build.DEVICE,
                        Build.MODEL,
                        (byDevice + byModel).distinct(),
